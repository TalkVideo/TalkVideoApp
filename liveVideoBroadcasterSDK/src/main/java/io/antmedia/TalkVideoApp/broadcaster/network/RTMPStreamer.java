package io.antmedia.TalkVideoApp.broadcaster.network;
...
public class RTMPStreamer extends Handler implements IMediaMuxer  {
        ...
        switch (msg.what) {
            ...
            case SEND_VIDEO: {
                ...
                if ((msg.arg2 >= mLastReceivedVideoFrameTimeStamp) && (msg.arg1 > 0)) {
                    ...
                    Log.w(TAG, "discarding video packet because time stamp is older  than last packet or data lenth equal to zero");
                }
                sendFrames();
            }
            break;
            case STOP_STREAMING:
                Log.w(TAG, "STOP_STREAMING Called");
                finishframes();
                // Send EOF Character?
                // writeEOF();
                close();
    ...
    public void writeAudio(byte[] data, int size, int presentationTime)
    {
        ...
        Log.d(TAG, "writeAudio size: " + size + " time:" + presentationTime);
    }
    ...
    public void writeVideo(byte[] data, int length, int presentationTime)
    {
        ...
        Log.d(TAG, "writeVideo size: " + length + " time:" + presentationTime);
    }
    ...
    public void writeEOF()
    {
        ...
        String eof = "\u004a";
        byte[] data =  eof.getBytes(StandardCharsets.US_ASCII);

        // Message message = obtainMessage(IMediaMuxer.SEND_VIDEO, data);
        Message message = obtainMessage(RTMPStreamer.STOP_STREAMING);

        Log.d(TAG, "writeEOF");
        message.arg1 = data.length;
        message.arg2 = 999999;
        sendMessage(message);
    }
