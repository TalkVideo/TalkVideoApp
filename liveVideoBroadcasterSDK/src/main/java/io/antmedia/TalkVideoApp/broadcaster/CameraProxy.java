            ...
            try {
                switch (msg.what) {
                    case OPEN_CAMERA:
                        ...
                        Log.i(TAG, "Camera Parameters On OPEN_CAMERA");


                        Log.i(TAG,_parameters.flatten().replace(';', '\n'));
                    ...
                    case SET_PARAMETERS:
                        // Log.i(TAG,((Camera.Parameters) msg.obj).flatten().replace(';', '\n'));
                        // _camera.setParameters((Camera.Parameters)msg.obj);

                        String NewParams = _parameters.flatten();

                        // NewParams = NewParams.replace("whitebalance=auto", "whitebalance=warm-fluorescent");
                        // NewParams = NewParams.replace("zoom=0", "zoom=99");
                        // NewParams = NewParams.replace("exposure-compensation=0", "exposure-compensation=2");
                        // NewParams = NewParams.replace("rotation=0", "rotation=90");
                        // NewParams = NewParams.replace("preview-size=1280x720", "preview-size=160x120");
                        // NewParams = NewParams.replace("video-size=1280x720", "video-size=1280x960");
                        // NewParams = NewParams.replace("effect=none", "effect=mono");

                        Log.i(TAG, "Camera Parameters Being Set On SET_PARAMETERS");
                        Log.i(TAG,NewParams.replace(';', '\n'));

                        _parameters.unflatten(NewParams);
